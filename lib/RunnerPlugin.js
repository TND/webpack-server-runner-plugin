...
WebpackServerRunnerPlugin.prototype.onError = function (error) {
    if (error.syscall !== 'listen') {
        ...
        console.error(colors.red.bold(prefix +
            ' Port ' + this.normalizedPort  + 'requires elevated privileges'
        ));
        process.exit(1);
    } else if (error.code === 'EADDRINUSE') {
        console.error(colors.red.bold(prefix +
            ' Port ' + this.normalizedPort + 'is already in use'
        ));
        process.exit(1);
    } else {
        throw error;
...
WebpackServerRunnerPlugin.prototype.update = function () {
    ...
    console.info(colors.green.bold(prefix + (firstTime ?
        (' Server listening on port ' + this.normalizedPort) :
        (' Server reloaded on port ' + this.normalizedPort)
    )));
};
